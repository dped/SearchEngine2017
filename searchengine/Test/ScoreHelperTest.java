...
public class ScoreHelperTest {
    ...
    public void setUp() throws Exception{
        ...
        sites.add(w1);  // add all sites to the ArrayList
        sites.add(w2);
        sites.add(w3);
        sites.add(w4);
        sites.add(w5);
        idx.build(sites);  // build websites from the ArrayList, returns a map
    }

    @Test
    public void tfScoreTest()  {
        TfScore tf = new TfScore();  // instance of tf score
        QueryHandler qh = new QueryHandler(idx, tf); // instance of query handler
        ...
        // Therefore
        List<Website> expectedOutcome = new ArrayList<>(); // based on word 20
        ...
        assertEquals(3.00, tf.getScore("word1", w1, idx), 0.001); // word found more than once on the site, 3 occurrences.
